            ...
            Ok(generate_context) => {
                ...
                if stage.is_error() {
                    let _ = db
                        .update_stage_task(
                            &task.id,
                            crate::stage_service::stage_service::ExecutorError::Error.into(),
                            "",
                        )
                        .await;
                } else {
                    let result =
                        prover::provers::read_file_bin(&generate_context.final_path).unwrap();
                    let _ = db
                        .update_stage_task(
                            &task.id,
                            crate::stage_service::stage_service::ExecutorError::NoError.into(),
